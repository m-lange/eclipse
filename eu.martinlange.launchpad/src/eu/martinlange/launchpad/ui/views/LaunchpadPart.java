...
public class LaunchpadPart extends CommonNavigator {
	...
	private void initConfigurationTree(final IMemento memento) {
		ISafeRunnable code = new ISafeRunnable() {
			@Override
			public void run() throws Exception {
				ConfigurationTree.INSTANCE.restoreState(memento);
			}


			@Override
			public void handleException(Throwable exception) {
				ConfigurationTree.INSTANCE.restoreState(null);
			}
		};
		SafeRunner.run(code);
	}
	...
	public void saveState(IMemento memento) {
		...
		ConfigurationTree.INSTANCE.saveState(memento);
	}
	...
	protected Object getInitialInput() {
		if (fCategorized)
			return ConfigurationTree.INSTANCE.getRoot();
		else
